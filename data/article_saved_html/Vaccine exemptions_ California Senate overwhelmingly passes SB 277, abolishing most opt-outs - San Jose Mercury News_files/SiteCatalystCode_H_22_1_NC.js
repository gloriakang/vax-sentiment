...
s.linkInternalFilters = "javascript:," + document.location.host + ",adclick.g.doubleclick.net"
s.linkLeaveQueryString = false
s.linkTrackVars = "None"
s.linkTrackEvents = "None"
s.trackingServer = "mngi.112.2o7.net"
loadsrc("http://extras.mnginteractive.com/live/js/omniture/plugins_min.js", "omn_plugins");
function forceVariables() {
    var old_gi = window.s_gi;
    window.s_gi =
    function(un, pg, ss) {
        var s = old_gi(un, pg, ss);
        window.omn_s_obj = s
        var old_T = s.t;
        var old_TL = s.tl;
        s.t = function(vo, id) {
            if(s!=window.s){window.s = s}
            trackFrame(false);
            window.s.prop1 = "D=g";
            AppendPageEvent("event1", false)
            if (inIframe()) {
                //bandaid to prevent tracking 404 from
                //badly implemented ad containers
                var u = document.location.href
                u = u.toLowerCase()
                if (u.indexOf("/eyeblaster/") == -1 && u.indexOf("dartiframe.html") == -1) {
                    return peekaboo(vo, id);
                }
                return ''
            } else {
                return peekaboo(vo, id);
            }
            function peekaboo(vo, id) {
                //unbind then re-bind the funtion
                //to avoid error
                var me = this.s.t;
                this.s.t = old_T;
                var result = this.s.t(vo, id);
                old_T = this.s.t;
                this.s.t = me;
                return result
            }
        }
        s.tl = function(o, t, n, vo) {
            if(s!=window.s){window.s = s}
            trackFrame(true);
            window.s.prop1 = "D=g";
            return peekaboo(o, t, n, vo);
            function peekaboo(o, t, n, vo) {
                //unbind then re-bind the funtion
                //to avoid error
                var me = this.s.tl
                this.s.tl = old_TL;
                var result = this.s.tl(o, t, n, vo);
                old_TL = this.s.tl;
                this.s.tl = me;
                return result
            }
...
function AppendPageEvent(e, isTL) {
    var eStr = null;
    var eStrA = null;
    if (s.events) {
        if (s.events.indexOf(e) == -1) {
            eStr = s.events;
            eStrA = eStr.split(",");
            eStrA.push(e);
            s.events = eStrA.join(",");
    ...
    if (isTL) {
        if (s.linkTrackEvents) {
            if (s.linkTrackEvents.indexOf(e) == -1) {
                eStr = s.linkTrackEvents;
                eStrA = eStr.split(",");
                eStrA.push(e);
                s.linkTrackEvents = eStrA.join(",");
            }
        } else {
            s.linkTrackEvents = e;
        }
...
function trackFrame(isTL) {
    if (inIframe()) {
        AppendPageEvent("event27", false)
        if (isTL) {
            AppendPageEvent("event27", true)
        }
    }
}
function inIframe() { if (top === self) { return false } return true }
