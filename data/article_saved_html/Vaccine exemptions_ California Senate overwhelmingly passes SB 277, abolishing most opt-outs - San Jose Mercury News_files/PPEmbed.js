...
var _ndnBuyerId = 'sanjosemercurynews_90757';
var distributorId = '90757';
var partnerId = '838';
...
targetsMap = function(url) {
    ...
    for (i=3; i < splits.length ; i++) {
      var upt1 = "" + splits.slice(i,i+1);
      if(upt1.slice(0,3) == "ci_"){
        newarr[newarr.length] = upt1;
      }else{
...
if(splits[2] == 'blogs.denverpost.com'){
      this.denver.target = uparts[0]+'*';
}else{
      this.denver.target = (newarr.join('/')).slice(0,-1);
}
    this.denver.shortDesc = "Ignore everything except article ID that begins with ci_";
 ...
 getPlacements().done(function(placements){				
    ...
    for(var i=0;i<jsonPlacements.length && recIndex<jsonRecommendations.length;i++){
      if(jsonPlacements[i].render_app=='dynamic' || (jsonPlacements[i].render_app=='wirematch' && jsonRecommendations[recIndex].score>=100)){
        var dynVID = (typeof(jsonRecommendations[recIndex]) !== 'undefined') ? jsonRecommendations[recIndex].id : "";
