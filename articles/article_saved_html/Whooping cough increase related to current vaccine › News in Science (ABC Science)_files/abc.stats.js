/**
 * @namespace   ABC.Stats
 * @description Assigns a stats profile based on a javascript variable set in Core Media, or by using 
 *              the top-level domain or directory, and loads the appropriate webtrends.load.profile script.
 * @version     2015-07-09
 * @author      Geoff Pack
 *
 */
...
ABC.Stats = function() {
	...
	var mobileStr = (location.host.indexOf('mobile.') == 0) ? '.mobile' : '';
	...
	function getProfile() {
		...
		switch (host) {
			...
			case 'triplejunearthed.com.au':
			case 'hottest100.triplej.net.au':	profile = 'triplej'; break;
			...
			default:
				switch (directories[1]) {
					...
					case 'spoke': 			profile = 'spoke'; break;
					...
					// mental as
					case 'mentalas': 		profile = 'mentalas'; break;
	...
	if (typeof window.abcContentProfile === 'undefined') {
		window.abcContentProfile = getProfile();
	}
	
	// load Webtrends
	var s, s2;
	s=document.createElement("script");s.async=true;s.src="http://www.abc.net.au/res/libraries/stats/webtrends-10.2/webtrends.load."+window.abcContentProfile+mobileStr+".js";  
	s2=document.getElementsByTagName("script")[0];s2.parentNode.insertBefore(s,s2);
