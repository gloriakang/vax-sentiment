...
var db=p.document,fb=db&&z?$a()||("CSS1Compat"==db.compatMode?parseInt(ab,10):5):void 0;!Wa&&!z||z&&9<=Number(fb)||Wa&&cb("1.9.1");var gb=z&&!cb("9");function ib(a){return a?new jb(kb(a)):pa||(pa=new jb)}
...
function Nd(a,b,c,d,e,f){Ia(c,R(a,"tip-reverse"),f);var g=0;f&&(g=1);a=fd(b);f=new y((a.width-10)/2,f?a.height:0);var h=kb(b),k=new y(0,0),l;l=h?kb(h):document;var m;(m=!z||9<=Number(fb))||(m=ib(l),m=qb(m.f));b!=(m?l.documentElement:l.body)&&(l=dd(b),m=ib(h).f,h=m.scrollingElement?m.scrollingElement:!Xa&&qb(m)?m.documentElement:m.body||m.documentElement,m=m.parentWindow||m.defaultView,h=z&&cb("10")&&m.pageYOffset!=h.scrollTop?new y(h.scrollLeft,h.scrollTop):new y(m.pageXOffset||h.scrollLeft,m.pageYOffset||
h.scrollTop),k.x=l.left+h.x,k.y=l.top+h.y);f=new y(k.x+f.x,k.y+f.y);f=f.clone();k=(g&8&&"rtl"==cd(c,"direction")?g^4:g)&-9;g=fd(c);l=g.clone();h=f.clone();l=l.clone();0!=k&&(k&4?h.x-=l.width+0:k&2&&(h.x-=l.width/2),k&1&&(h.y-=l.height+0));f=new bd(0,0,0,0);f.left=h.x;f.top=h.y;f.width=l.width;f.height=l.height;l=new y(f.left,f.top);l instanceof y?(k=l.x,l=l.y):(k=l,l=void 0);c.style.left=ed(k);c.style.top=ed(l);l=new Ta(f.width,f.height);if(!(g==l||g&&l&&g.width==l.width&&g.height==l.height))if(g=
l,f=kb(c),f=ib(f),k=qb(f.f),!z||cb("10")||k&&cb("8"))f=c.style,Wa?f.MozBoxSizing="border-box":Xa?f.WebkitBoxSizing="border-box":f.boxSizing="border-box",f.width=Math.max(g.width,0)+"px",f.height=Math.max(g.height,0)+"px";else if(f=c.style,k){z?(k=id(c,"paddingLeft"),l=id(c,"paddingRight"),h=id(c,"paddingTop"),m=id(c,"paddingBottom"),k=new M(h,l,m,k)):(k=N(c,"paddingLeft"),l=N(c,"paddingRight"),h=N(c,"paddingTop"),m=N(c,"paddingBottom"),k=new M(parseFloat(h),parseFloat(l),parseFloat(m),parseFloat(k)));
if(!z||9<=Number(fb))l=N(c,"borderLeftWidth"),h=N(c,"borderRightWidth"),m=N(c,"borderTopWidth"),G=N(c,"borderBottomWidth"),l=new M(parseFloat(m),parseFloat(h),parseFloat(G),parseFloat(l));else{l=kd(c,"borderLeft");h=kd(c,"borderRight");m=kd(c,"borderTop");var G=kd(c,"borderBottom");l=new M(m,h,G,l)}f.pixelWidth=g.width-l.left-k.left-k.right-l.right;f.pixelHeight=g.height-l.top-k.top-k.bottom-l.bottom}else f.pixelWidth=g.width,f.pixelHeight=g.height;g=window.document;g=qb(g)?g.documentElement:g.body;
f=new Ta(g.clientWidth,g.clientHeight);1==c.nodeType?(c=dd(c),l=new y(c.left,c.top)):(c=c.changedTouches?c.changedTouches[0]:c,l=new y(c.clientX,c.clientY));c=fd(d);h=Math.floor(c.width/2);g=!!(f.height<l.y+a.height);a=!!(l.y<a.height);k=!!(l.x<h);f=!!(f.width<l.x+h);l=(c.width+3)/-2- -5;b=Q(b,"force-tooltip-direction");if("left"==b||k)l=-5;else if("right"==b||f)l=20-c.width-3;b=Math.floor(l)+"px";d.style.left=b;e&&(e.style.left=b,e.style.height=c.height+"px",e.style.width=c.width+"px");return!(g||
a)}
...
function De(a,b,c){function d(a,b){if(G)return 0;Sd.clearTimeout(m);ra.push.apply(ra,S);var d=((Zd||{}).config||{}).update;d?d(f):f&&W(Y,"cu",[]).push(f);if(b){ee("me0",a,eb);try{Ae(b,c,l)}finally{ee("me1",a,eb)}}return 1}
a=Yd(a)||[];var e=b.callback,f=b.config,g=b.timeout,h=b.ontimeout,k=b.onerror,l=void 0;"function"==typeof k&&(l=k);var m=null,G=!1;if(g&&!h||!g&&h)throw"Timeout requires both the timeout parameter and ontimeout parameter to be set";var k=W(be(c),"r",[]).sort(),ra=W(be(c),"L",[]).sort(),eb=[].concat(k);0<g&&(m=Sd.setTimeout(function(){G=!0;h()},g));
...
if(!a)return 0;ee("ml1",S,eb);0<fa&&ea[fa-1]?ea[fa]=function(){c(b)}:c(b)};
if(S.length){var $b="loaded_"+Y.I++;Zd[$b]=function(a){ea[fa](a);Zd[$b]=null};
a=me(c,S,"gapi."+$b,k);k.push.apply(k,S);ee("ml0",S,eb);b.sync||Sd.___gapisync?xe(a):ye(a)}else ea[fa](Vd)}else d(S)&&e&&e()}
